...
int sendStart() {
    ...
    int n = read(sockfd, buf, sizeof(buf));
    buf[n] = '\0';
    char gameNoStr[10], playerNoStr[3];
...
void sendReady() {
    ...
    addrUdp.sin_port = htons(UDP_PORT);
    addrUdp.sin_addr.s_addr = inet_addr(SERVER_IP);
    socklen_t len = sizeof(addrUdp);
    int sockfdUdp = socket(AF_INET, SOCK_DGRAM, 0);
    std::string msg = "ready ";
    msg = msg + std::to_string(gameNo) + " " + std::to_string(playerNo) + "\n";
    const char* buf = msg.c_str();
    sendto(sockfdUdp, buf, strlen(buf), 0, (sockaddr*)&addrUdp, sizeof(addrUdp));
    char rebuf[BUFFER_SIZE];
    //recvfrom(sockfdUdp, rebuf, BUFFER_SIZE, 0, (sockaddr*)&addrUdp, &len);
    close(sockfdUdp);
}
